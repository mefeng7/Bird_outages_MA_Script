...
RF_town_assess_bind<-do.call("rbind",RF_town_assess)%>%
  mutate(Species=rep(name_list$sp_sci,each=2),
         Common.Name= rep(name_list$sp_name,each=2))
RF_town_assess_sum<-RF_town_assess_bind%>%
  group_by(model)%>%
...
write.csv(RF_town_assess_bind,paste0("Outputs/RF_perfomance_indiv.csv"))
