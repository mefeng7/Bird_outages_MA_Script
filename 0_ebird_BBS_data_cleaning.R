...
name_list <- list(
  ...
  sp_sci = c("Agelaius phoeniceus","Quiscalus quiscula","Molothrus ater", "Buteo jamaicensis","Pandion haliaetus", "Melanerpes carolinus",
             ...
             "Cathartes aura", "Dryocopus pileatus","Corvus brachyrhynchos",
             "Myiopsitta monachus"),
  #day of the year with peak breeding season detection frequency for each species
  peak = c(121,145,121,289,205,121,109,43,121,181,193,157,103,115,NA,NA)) 
...
sp_ebird2<-purrr::map(sp_ebird, function(x) {st_join(x,ma_towns, join = st_intersects,
                                                     left = FALSE)%>%
    st_drop_geometry()})


ebird_towns<-sp_ebird2
